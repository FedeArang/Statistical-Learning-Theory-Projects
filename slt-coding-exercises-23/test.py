...
class DeterministicAnnealingClustering(skl.base.BaseEstimator,
    ...
    def fit(self, samples):
        ...
        while self.T > self.eps_T:
            ...
            #if idx%20==0:
                #print(idx, self.T, self.cluster_centers, self.cluster_probs)
            ...
            while diff>10e-6:
                ...
                for i in range(self.K):
                    ...
                    self.cluster_centers[i]=1/self.marginal_probs[i]*1/n_samples*(sum(samples[j]*self.cluster_probs[j, i] for j in range(n_samples)))
            ...
            if self.K < self.K_max:
                for i in range(self.K):
                    ...
                    cluster_posterior=1/n_samples*1/self.marginal_probs[i]*self.cluster_probs[:, i]

                    #cluster_covariance=cluster_posterior*(np.dot((n_samples-self.cluster_centers[i]).sum(axis=)
                    ...
                    eigenvalues, eigenvectors=np.linalg.eig(cluster_covariance)
                    critical_temperature=2*max(eigenvalues)

                    if self.T<critical_temperature:

                        splitting_cluster_center = self.cluster_centers[i, :].copy()

                        self.K+=1

                        new_centroid=self.cluster_centers[i, :]+np.random.normal(0, self.noise, n_features)
                        self.cluster_centers=np.vstack((self.cluster_centers, new_centroid))

                        new_p_y=self.marginal_probs[i]/2
                        self.marginal_probs[i]=self.marginal_probs[i]/2
                        self.marginal_probs=self.marginal_probs.tolist()
                        self.marginal_probs.append(new_p_y)
                        self.marginal_probs=np.array(self.marginal_probs)
                        ...
                        self.bifurcation_tree.create_node(identifier=splitting_cluster_identifier,
                                                          ...
                                                          parent='parent')
                        distance = self.distance(splitting_cluster_center, new_centroid)
                        ...
                        self.bifurcation_tree.create_node(identifier=self.K-1,
                                                          data={'cluster_id': self.K-1, 'distance': distance_list_new, 'centroid_position': new_centroid, 'direction': 'left'},
                                                          parent='parent')
    ...
    def _calculate_cluster_probs(self, dist_mat, temperature):
        ...
        probs=np.zeros(dist_mat.shape)

        un_normalized_probs=self.marginal_probs*np.exp(-1/temperature*np.square(dist_mat))

        Z= un_normalized_probs.sum(axis=1)    #calculate the normalization constant
        Z=Z.reshape((dist_mat.shape[0], 1))

        probs=un_normalized_probs/Z #we normalize the probability 
    ...
    def get_distance(self, samples, clusters):
        ...
        for i in range(samples.shape[0]):
            distance_matrix[i, :]=((clusters-samples[i, :])**2).sum(axis=1)
